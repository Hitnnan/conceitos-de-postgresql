...
AS $$
BEGIN
    -- Realiza insert. Caso contrário atualiza, como regra de negócio, usando a expressão em conflito -> atualizar
    INSERT INTO banco (numero, nome, ativo)
    VALUES (p_numero, p_nome, p_ativo)
    ON CONFLICT (numero) DO UPDATE SET nome = p_nome, ativo = p_ativo;
    
    -- É obrigatório retornar uma tabela
    RETURN QUERY
        SELECT numero, nome, ativo
        FROM banco
...
BEGIN
    -- Valida se o banco existe, e se está ativo
    SELECT INTO variavel_banco_numero numero
    FROM vw_bancos
    WHERE numero = p_banco_numero
    AND ativo IS TRUE;
    
    -- Se existe e está ativo, então prossegue, inserindo uma agência
...
BEGIN
    ...
    -- Insert com tratamento de valor nulo
